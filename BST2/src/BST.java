...
protected int helper(BSTNode<T> p) {
    	if (p != null) {
    		nodes.add(p.el);
        	return helper(p.left)+helper(p.right)+1;
    	}else
    		return 0;
	}
public int rangeCounter(T t1, T t2) {
	...
	int result = nodes.indexOf(t2)-nodes.indexOf(t1)-1;
		return result;
	}
}
